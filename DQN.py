...
class DQN:
    ...
    def train(self, n_episodes:int = 1000,validate_every:int = 100, n_validation_episodes:int = 10, n_test_episodes:int = 10,save_every:int = 100):
        ...
        val_reward_history, val_std_history = np.array([]), np.array([])
        ...
        for episode in range(n_episodes):
            ...
            if i !=0:
                ...
                train_reward_history = np.append(train_reward_history, total_reward)
                train_loss_history = np.append(train_loss_history, loss/i)
            if episode % validate_every == validate_every - 1:
                ...
                val_reward_history = np.append(val_reward_history,mean_reward)
                val_std_history = np.append(val_std_history,std_reward)
    ...
    def _optimize_model(self):
        ...
        if not enough_samples:
            ...
            for i in range(self.batch_size): #select the q value for each sample
                q[i] = outputs[i][actions[i]]

            outputs_next_states = self.model(next_states)
            q_next_states, _ = torch.max(outputs_next_states, dim=1) #select q next using argmax policy
            targets = rewards + self.gamma*q_next_states * (1 - dones.float())
    ...
    def _sample_action(self, state:np.ndarray
        ...
        if temp < epsilon:
            action = int(random.choice([0, 1, 2, 3, 4]))
        else:
...
class HardUpdateDQN(DQN):
    ...
    def _optimize_model(self):
        ...
        if not enough_samples:
            ...
            self._update_model()
            outputs_next_states = self.target_model(next_states)
            q_next_states, _ = torch.max(outputs_next_states, dim=1) #select q next using argmax policy
            targets = rewards + self.gamma*q_next_states * (1 - dones.float())
            targets = targets.detach().to(self.device)            
            loss = self.loss_fn(q, targets)
